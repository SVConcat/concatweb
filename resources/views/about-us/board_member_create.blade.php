<x-layout>
    <div class="bg-white p-6 rounded-xl shadow-lg w-full max-w-5xl mt-5 mb-5">
        <h1 aria-label="Bestuurslid aanmaken"
        ...
        <form method="POST" action="{{ route('board-members.store') }}" enctype="multipart/form-data"
            ...
            <div>
                ...
                <textarea name="bio" id="bio" rows="6"
                          ...
                          aria-label="Voer een biografie in voor dit bestuurslid">{{ old('bio') }}</textarea>
                @error('bio')
                <div class="text-red-500 text-l mt-1 font-bold">{{ $message }}</div>
                @enderror
            </div>
