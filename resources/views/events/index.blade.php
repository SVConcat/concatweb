<x-layout>
    <div class="bg-white p-6 rounded-xl shadow-lg w-full max-w-5xl mt-5 mb-5">
            ...
            @if(auth()->user()->isAdmin())
                <div class="flex justify-end my-4">
                    <a href="{{ route('events.create') }}"
                       ...
                       aria-label="Voeg een nieuw evenement toe">
        ...
        <form method="GET" action="{{route('events.index') }}"
              class="mb-6 flex flex-col sm:flex-row items-center gap-4 justify-center">
            <div>
                ...
                <select name="categorie" id="categorie"
                    ...
                    <option value="blokborrel" {{ $categorieFilter === 'blokborrel' ? 'selected' : '' }}>Blokborrel
                    </option>
                    <option value="education" {{ $categorieFilter === 'education' ? 'selected' : '' }}>Education
                    </option>
                </select>
            </div>
            @auth
            ...
            {{-- Past events button --}}
            <div>
                <a href="{{ route('events.index', ['afgelopen' => 'true', 'categorie' => $categorieFilter]) }}"
        ...
        <div class="flex flex-col flex-wrap my-4">
                ...
                @foreach($events as $event)
                    <div class="bg-white border border-gray-200 rounded-lg shadow-md overflow-hidden">
                        <a href="{{ route('events.show', $event->id) }}"
                           ...
                           aria-label="Bekijk details van {{ $event->titel }}">
                            @if(isset($event->afbeelding))
                                <img src="{{ Storage::url($event['afbeelding']) }}"
                                     alt="Afbeelding van {{ $event->titel }}. Datum: {{ $event->formatted_start_datum }} tot {{ $event->formatted_eind_datum }} in {{ $event->locatie }}"
                        ...
                        <div class="p-5">
                            ...
                            @auth
                                @if($event->isUserRegistered(auth()->id()))
                                    <span
                                        class="inline-block mb-2 bg-green-100 text-green-700 text-xs font-medium mr-2 px-2.5 py-0.5 rounded-full">
                                        Ingeschreven
                                    </span>
                                @else
                                    <span
                                        class="inline-block mb-2 bg-red-100 text-red-700 text-xs font-medium mr-2 px-2.5 py-0.5 rounded-full">
                                        Niet ingeschreven
                                    </span>
                                @endif
                            @endauth
                            ...
                            @if(isset($event->datum))
                                <div class="flex items-center text-gray-500 mb-4 mt-3">
                                    ...
                                    <span
                                        class="text-lg font-bold">{{ $event->datum }}, {{ $event->starttijd }} - {{ $event->eindtijd }}</span>
                                </div>
                            @endif
                            ...
                            <a href="{{ route('events.show', $event->id) }}"
                               ...
                               aria-label="Lees meer over {{ $event->titel }}. Datum van {{ $event->formatted_start_datum }} tot {{ $event->formatted_eind_datum }} in {{ $event->locatie }}">
                                Lees meer...
                            </a>
                        </div>
                    </div>
            ...
            {{-- Pagination --}}
            <div id="pagination-container" class="mt-6 text-center" role="navigation"
