<x-layout>
    <div class="bg-white p-6 rounded-xl shadow-lg w-full max-w-5xl mt-5 mb-5">
        ...
        <form method="POST" action="{{ route('gallery.store') }}" enctype="multipart/form-data" class="mt-4 space-y-4"
            ...
            <div>
                <label class="block text-l font-bold mb-2">
                    Koppel aan evenement(en)
                </label>

                <div
                    class="flex flex-col gap-2 max-h-[320px] overflow-y-auto pr-1 border border-purple-300 rounded-lg bg-purple-50 p-2">
                    @foreach($evenementen as $evenement)
                        <label class="flex items-center gap-3 p-3 rounded-md border border-purple-300 bg-purple-100 text-purple-700 cursor-pointer hover:bg-purple-200 transition">
                            <input
                                type="checkbox"
                                name="evenementen[]"
                                value="{{ $evenement->id }}"
                                class="accent-purple-600 shrink-0"
                                {{ collect(old('evenementen'))->contains($evenement->id) ? 'checked' : '' }}
                            >
                            <span class="text-sm font-medium leading-tight">{{ $evenement->titel }}</span>
                        </label>
                    @endforeach
                </div>

                <p class="text-xs text-gray-500 mt-1">
                    Je kunt meerdere evenementen selecteren
                </p>

                @error('evenementen')
                <div class="text-red-500 text-l mt-2 font-bold">
                    {{ $message }}
                </div>
                @enderror
            </div>
